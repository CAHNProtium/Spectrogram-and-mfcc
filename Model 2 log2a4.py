...
x_dev = []
data1 = pd.read_csv("path/specdevlog2a4.csv", header=None)
...
x_eval = []
data1 = pd.read_csv("path/specevallog2a4.csv", header=None)
...
for x in range(1):
  ...
  model.save('path/AR1log2a4-'+str(x+1)+'.h5')
  #----------------------------------------EVALUATION----------------------------------------
  print("\nEvaluacion "+ str(x+1)+":")
  results = model.evaluate(x_eval, y_eval, verbose = 1)
  trainaccu.append(history.history['accuracy'][9])
  devaccu.append(history.history['val_accuracy'][9])
